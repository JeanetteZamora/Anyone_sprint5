# Sprint 05 Project
> Vehicle classification from images, Jeanette Zamora -ver 
...
### Run Docker [OK] --> docker run --rm --net host -it -v $(pwd):/home/app/src --workdir /home/app/src s05_project bash
...
### Run Unit test [PENDIENTE]

```bash
$ pytest tests/
```

## 2. Prepare your data [OK]
...
Then, you should be able to run the script `scripts/prepare_train_test_dataset.py`. It will format your data in a way Keras can use for training our CNN model. [OK]
...
You can check the file `experiments/config_example.yml` to get an idea on all the configurations you can set for an experiment. [ok]
...
- `utils.load_config()`: Takes as input the path to an experiment YAML configuration file, loads it and returns a dict.[ok]
- `resnet50.create_model()`: Returns a CNN ready for training or for evaluation, depending on the input parameters received. Part of coding this functions will require you to create the layers of your first CNN with Keras.[ok]
- `data_aug.create_data_aug_layer()`: Used by `resnet50.create_model()`. This function adds data augmentation layers to our model that will be used only while training. [ok]
