...
def main(data_folder, labels, output_data_folder):
    ...
    car_dataset_labels = pd.read_csv(labels, header=[0])

    #   2. Iterate over each row in the CSV, create the corresponding
    #      train/test and class folders
    subset = car_dataset_labels['subset'].unique().tolist()
    folder_subset = [os.makedirs(os.path.join(output_data_folder, os.fspath(x)),exist_ok=True) for x in subset]

    for row in car_dataset_labels.itertuples():
        if row.subset == 'test':
            folder_class_ = os.path.join(output_data_folder, 'test', row[2])
            os.makedirs(folder_class_, exist_ok= True)
        
        elif row.subset == 'train':
            folder_class_ = os.path.join(output_data_folder, 'train', row[2])
            os.makedirs(folder_class_, exist_ok= True)
    ...
    #      use `os.link()` to avoid wasting disk space with duplicated files
        ...
        origin = os.path.join(data_folder, f"{row[1]}")
        destination = os.path.join(folder_class_, f"{row[1]}")
